...
# -------- Optical Flow (DISOpticalFlow) --------
def compute_dis_flow(prev_img, next_img):
    """
    Compute the optical flow between two images using the DISOpticalFlow algorithm.
    
    Args:
        prev_img (np.array): Previous image (normalized between 0 and 1).
        next_img (np.array): Next image (normalized between 0 and 1).

    Returns:
        tuple: Optical flow in horizontal and vertical directions (u, v).
    """
    prev = (prev_img * 255).astype(np.uint8)
    next = (next_img * 255).astype(np.uint8)
    prev_gray = cv2.cvtColor(prev, cv2.COLOR_RGB2GRAY) if prev.ndim == 3 else prev
    next_gray = cv2.cvtColor(next, cv2.COLOR_RGB2GRAY) if next.ndim == 3 else next
...
class CloudDataset(Dataset):
    ...
    def __init__(self, image_dir, T_in=3, ext='jpg'):
        ...
        self.flows = [np.stack(compute_dis_flow(self.images[i], self.images[i + 1]), axis=0)
