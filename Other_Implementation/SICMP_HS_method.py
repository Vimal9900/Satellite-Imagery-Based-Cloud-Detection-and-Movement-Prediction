...
def compute_Horn_Schunck_flow(images, alpha=15, num_iter=100):
    """
    Compute optical flow fields for a sequence of grayscale images using Horn-Schunck.
    Returns a list of tuples (u, v) for each consecutive image pair.
    """
    flows = []
...
class CloudDataset(Dataset):
    ...
    def __init__(self, image_dir, T_in=3, ext='jpg'):
        ...
        self.flows = [np.stack(compute_Horn_Schunck_flow(self.images[i], self.images[i + 1]), axis=0)
