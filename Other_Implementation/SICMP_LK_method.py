...
# -------- Optical Flow (Lucas_Kanade) --------
def compute_lucas_kanade_flow(prev_img, next_img):
    ...
    points_prev = np.vstack((x.ravel(), y.ravel())).T.astype(np.float32)
    points_next, status, _ = cv2.calcOpticalFlowPyrLK(prev_gray, next_gray, points_prev, None)

    u = np.zeros_like(prev_gray, dtype=np.float32)
    v = np.zeros_like(prev_gray, dtype=np.float32)

    for (pt_prev, pt_next, st) in zip(points_prev, points_next, status):
        if st[0] == 1:
            x0, y0 = pt_prev.astype(int)
            dx, dy = pt_next - pt_prev
            u[y0, x0] = dx
            v[y0, x0] = dy
...
class CloudDataset(Dataset):
    ...
    def __init__(self, image_dir, T_in=3, ext='jpg'):
        ...
        self.flows = [np.stack(compute_lucas_kanade_flow(self.images[i], self.images[i + 1]), axis=0)
